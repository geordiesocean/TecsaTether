...
void PrepareShutdown()
{
    ...
    util::ThreadRename("tecsatether-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), TECSATETHER_CONF_FILENAME));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), TECSATETHER_PID_FILENAME));
#endif
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all USDTX specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-mnconf=<file>", strprintf(_("Specify masternode configuration file (default: %s)"), TECSATETHER_MASTERNODE_CONF_FILENAME));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:10024"));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2023-%i The TecsaTether Coin Developers & TylerAnderson T.A"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    util::ThreadRename("tecsatether-loadblk");
...
/** Sanity checks
 *  Ensure that TecsaTether is running in a usable environment with all
...
#endif
    LogPrintf("TecsaTether version %s (%s)\n", version_string, CLIENT_DATE);
}

/** Initialize tecsatether.
...
bool AppInit2()
{
    ...
    // Sanity check
    if (!InitSanityCheck())
        return UIError(_("Initialization sanity check failed. TecsaTether is shutting down."));

    std::string strDataDir = GetDataDir().string();

    // Make sure only a single TecsaTether process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return UIError(strprintf(_("Cannot obtain a lock on data directory %s. TecsaTether is probably already running."), strDataDir));
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                //TecsaTether specific: zerocoin and spork DB's
                ...
                // TecsaTether: load previous sessions sporks if we have them.
                ...
                if (fReindexMoneySupply) {
                    LOCK(cs_main);
                    // Skip zUSDTX if already reindexed
                    RecalculateUSDTXSupply(1, fReindexZerocoin);
                }
