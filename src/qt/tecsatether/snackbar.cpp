...
#include "qt/tecsatether/forms/ui_snackbar.h"
#include "qt/tecsatether/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(TECSATETHERGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &TECSATETHERGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
