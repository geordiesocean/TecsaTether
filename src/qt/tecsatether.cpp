...
#include "config/tecsatether-config.h"
#endif
...
#include "qt/tecsatether/splash.h"
#include "qt/tecsatether/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("tecsatether-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in tecsatether.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in tecsatether.qrc)
...
/** Class encapsulating TecsaTether Core startup and shutdown.
...
public:
    ...
    /// Get window identifier of QMainWindow (TECSATETHERGUI)
...
{
    window = new TECSATETHERGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &TECSATETHERGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &TECSATETHERGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(TECSATETHERGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(TECSATETHERGUI::DEFAULT_WALLET);
        ...
        // TecsaTether: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &TECSATETHERGUI::handlePaymentRequest);
        connect(window, &TECSATETHERGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. TecsaTether can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(tecsatether_locale);
    Q_INIT_RESOURCE(tecsatether);
    ...
    /// 6. Determine availability of data directory and parse tecsatether.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("TecsaTether"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("TecsaTether"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // tecsatether: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("TecsaTether didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
