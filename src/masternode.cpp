...
CAmount CMasternode::GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 1) {
        nSubsidy = 990000000 * COIN; 
    }
    else if (nHeight <= 350400001) { 
        nSubsidy = 0.02853881 * COIN;
    }
    else if (nHeight == 350400002) { 
        nSubsidy = 0.976 * COIN;
    }
    else { 
        nSubsidy = 0 * COIN;
...
{
  if (nHeight > 0) return GetBlockValue(nHeight) * 0 / 100;
}
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 USDTX tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 USDTX tx -> 1 confirmation
