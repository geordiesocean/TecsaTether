...
#include "zusdtx/zusdtxmodule.h"
#include "zusdtx/zusdtxwallet.h"
#include "zusdtx/zusdtxtracker.h"
...
enum WalletFeature {
    ...
    FEATURE_PRE_TECSATETHER = 61000, // inherited version..
...
// Possible states for zUSDTX send
enum ZerocoinSpendStatus {
    ZUSDTX_SPEND_OKAY = 0,                            // No error
    ZUSDTX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZUSDTX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZUSDTX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZUSDTX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZUSDTX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZUSDTX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZUSDTX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZUSDTX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZUSDTX_TXMINT_GENERAL = 9,                        // General errors in MintsToInputVectorPublicSpend
    ZUSDTX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZUSDTX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZUSDTX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZUSDTX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZUSDTX_SPENT_USED_ZUSDTX = 14,                      // Coin has already been spend
    ZUSDTX_TX_TOO_LARGE = 15,                         // The transaction is larger than the max tx size
    ZUSDTX_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void doZUsdtxRescan(const CBlockIndex* pindex, const CBlock& block, std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZUSDTX);
    ...
    bool CreateZUSDTXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    ...
    // zUSDTX wallet
    std::unique_ptr<CzUSDTXTracker> zusdtxTracker{nullptr};
    void setZWallet(CzUSDTXWallet* zwallet);
    CzUSDTXWallet* getZWallet();
