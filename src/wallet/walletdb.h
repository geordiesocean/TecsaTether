...
public:
    ...
    bool WriteZUSDTXSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZUSDTXSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZUSDTXSeed_deprecated(uint256& seed);
    bool EraseZUSDTXSeed();
    bool EraseZUSDTXSeed_deprecated();

    bool WriteZUSDTXCount(const uint32_t& nCount);
    bool ReadZUSDTXCount(uint32_t& nCount);
