...
#include "zusdtx/deterministicmint.h"
    ...
    if(IsCrypted()) {
        if (!IsLocked()) { //if we have password
            ...
            if (EncryptSecret(vMasterKey, kmSeed, hashSeed, vchSeedSecret)) {
                ...
                if (db.WriteZUSDTXSeed(hashSeed, vchSeedSecret)) {
                    return true;
                }
            }
            strErr = "encrypt seed";
        ...
        if (db.WriteZUSDTXSeed(hashSeed, ToByteVector(seed))) {
            return true;
        }
        strErr = "save zUSDTXseed to wallet";
    }
    //the use case for this is no password set seed, mint dzUSDTX,
...
bool CWallet::GetDeterministicSeed(const uint256& hashSeed, uint256& seedOut)
{
    ...
    if (IsCrypted()) {
        if(!IsLocked()) { //if we have password
            ...
            //read encrypted seed
            if (db.ReadZUSDTXSeed(hashSeed, vchCryptedSeed)) {
        ...
        // wallet not crypted
        if (db.ReadZUSDTXSeed(hashSeed, vchSeed)) {
...
void CWallet::doZUsdtxRescan(const CBlockIndex* pindex, const CBlock& block,
        std::set<uint256>& setAddedToWallet, const Consensus::Params& consensus, bool fCheckZUSDTX)
{
    //If this is a zapwallettx, need to read zUSDTX
    if (fCheckZUSDTX && consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_ZC)) {
...
bool CWallet::CreateZUSDTXOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint)
{
    ...
    zwallet->GenerateDeterministicZUSDTX(denomination, coin, dMint);
                                            ...
                                            const CCoinControl* coinControl)
{
    ...
    while (true) {
        ...
        if (!CreateZUSDTXOutPut(denomination, outMint, dMint)) {
            strFailReason = strprintf("%s: failed to create new zUSDTX output", __func__);
    ...
    //also assume that a zerocoinspend that is minting the change will not have any change that goes to USDTX
        ...
        std::list<std::pair<CTxDestination, CAmount>> addressesTo, CTxDestination* changeAddress)
{
    ...
    int nStatus = ZUSDTX_SPEND_ERROR;

    if (IsLocked()) {
        receipt.SetStatus("Error: Wallet locked, unable to create transaction!", ZUSDTX_WALLET_LOCKED);
        return false;
    }
    ...
    if (res.status != CWallet::CommitStatus::OK) {
        ...
        for (CZerocoinSpend spend : receipt.GetSpends()) {
            if (!walletdb.EraseZerocoinSpendSerialEntry(spend.GetSerial())) {
                receipt.SetStatus("Error: It cannot delete coin serial number in wallet", ZUSDTX_ERASE_SPENDS_FAILED);
            }
        ...
        for (auto& dMint : vNewMints) {
            if (!walletdb.EraseDeterministicMint(dMint.GetPubcoinHash())) {
                receipt.SetStatus("Error: Unable to cannot delete zerocoin mint in wallet", ZUSDTX_ERASE_NEW_MINTS_FAILED);
            }
        }
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        uint256 hashPubcoin = GetPubCoinHash(mint.GetValue());
        zusdtxTracker->SetPubcoinUsed(hashPubcoin, txidSpend);

        CMintMeta metaCheck = zusdtxTracker->GetMetaFromPubcoin(hashPubcoin);
        if (!metaCheck.isUsed) {
    ...
    receipt.SetStatus("Spend Successful", ZUSDTX_SPEND_OKAY);  // When we reach this point spending zUSDTX was successful
                                    ...
                                    CZerocoinSpendReceipt& receipt, libzerocoin::SpendType spendType, CBlockIndex* pindexCheckpoint)
{
    ...
    receipt.SetStatus(_("Transaction Mint Started"), ZUSDTX_TXMINT_GENERAL);
    ...
    for (auto &it : mapMintsSelected) {
        ...
        if (!GetTransaction(mint.GetTxHash(), txMint, hashBlock)) {
            receipt.SetStatus(strprintf(_("Unable to find transaction containing mint %s"), mint.GetTxHash().GetHex()), ZUSDTX_TXMINT_GENERAL);
            return false;
        } else if (mapBlockIndex.count(hashBlock) < 1) {
            // check that this mint made it into the blockchain
            receipt.SetStatus(_("Mint did not make it into blockchain"), ZUSDTX_TXMINT_GENERAL);
            return false;
        }
        ...
        if (outputIndex == -1) {
            receipt.SetStatus(_("Pubcoin not found in mint tx"), ZUSDTX_TXMINT_GENERAL);
            return false;
        }

        mint.SetOutputIndex(outputIndex);
        CTxIn in;
        if(!ZUSDTXModule::createInput(in, mint, hashTxOut, spendVersion)) {
            receipt.SetStatus(_("Cannot create public spend input"), ZUSDTX_TXMINT_GENERAL);
            return false;
        }
        vin.emplace_back(in);
    ...
    receipt.SetStatus(_("Spend Valid"), ZUSDTX_SPEND_OKAY); // Everything okay
        ...
        CTxDestination * changeAddress)
{
    ...
    int nStatus = ZUSDTX_TRX_FUNDS_PROBLEMS;
    ...
    if (nValue < 1) {
        receipt.SetStatus(_("Value is below the smallest available denomination (= 1) of zUSDTX"), nStatus);
        return false;
    }

    // Create transaction
    nStatus = ZUSDTX_TRX_CREATE;
    ...
    const int nMaxSpends = Params().GetConsensus().ZC_MaxPublicSpendsPerTx; // Maximum possible spends for one zUSDTX public spend transaction
    std::vector<CMintMeta> vMintsToFetch;
    if (vSelectedMints.empty()) {
        //  All of the zUSDTX used in the public coin spend are mature by default (everything is public now.. no need to wait for any accumulation)
        setMints = zusdtxTracker->ListMints(true, false, true, true); // need to find mints to spend
        if(setMints.empty()) {
        ...
        // Select the zUSDTX mints to use in this spend
    ...
    for (CZerocoinMint mint : vSelectedMints) {
        ...
        if (IsSerialInBlockchain(mint.GetSerialNumber(), nHeightSpend)) {
            ...
            if (!zusdtxTracker->HasSerialHash(hashSerial))
                return error("%s: tracker does not have serialhash %s", __func__, hashSerial.GetHex());

            CMintMeta meta = zusdtxTracker->Get(hashSerial);
            meta.isUsed = true;
            zusdtxTracker->UpdateState(meta);
    ...
    wtxNew.BindWallet(this);
    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            for (std::pair<CTxDestination,CAmount> pair : addressesTo){
                ...
                //add output to USDTX address to the transaction (the actual primary spend taking place)
                // TODO: check value?
            ...
            unsigned int nBytes = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
            if (nBytes >= MAX_ZEROCOIN_TX_SIZE) {
                receipt.SetStatus(_("In rare cases, a spend with 7 coins exceeds our maximum allowable transaction size, please retry spend using 6 or less coins"), ZUSDTX_TX_TOO_LARGE);
                return false;
            }
    ...
    receipt.SetStatus(_("Transaction Created"), ZUSDTX_SPEND_OKAY); // Everything okay
...
{
    if (fMatureOnly) {
        // This code is not removed just for when we back to use zUSDTX in the future, for now it's useless,
        ...
        CAmount nBalance = 0;
        std::vector<CMintMeta> vMints = zusdtxTracker->GetMints(true);
    ...
    return zusdtxTracker->GetBalance(false, false);
}

CAmount CWallet::GetUnconfirmedZerocoinBalance() const
{
    return zusdtxTracker->GetUnconfirmedBalance();
}
...
std::map<libzerocoin::CoinDenomination, CAmount> CWallet::GetMyZerocoinDistribution() const
{
    ...
    {
        LOCK(cs_wallet);
        std::set<CMintMeta> setMints = zusdtxTracker->ListMints(true, true, true);
...
void CWallet::setZWallet(CzUSDTXWallet* zwallet)
{
    this->zwallet = zwallet;
    zusdtxTracker = std::unique_ptr<CzUSDTXTracker>(new CzUSDTXTracker(this));
}

CzUSDTXWallet* CWallet::getZWallet()
{
    return zwallet;
}
...
{
    return zusdtxTracker->HasSerial(bnSerial);
}

bool CWallet::IsMyMint(const CBigNum& bnValue) const
{
    if (zusdtxTracker->HasPubcoin(bnValue))
        return true;
...
void CWallet::ReconsiderZerocoins(std::list<CZerocoinMint>& listMintsRestored, std::list<CDeterministicMint>& listDMintsRestored)
{
    ...
    for (CZerocoinMint mint : listMints) {
        ...
        if (!zusdtxTracker->UnArchive(hashPubcoin, false)) {
            LogPrintf("%s : failed to unarchive mint %s\n", __func__, mint.GetValue().GetHex());
        } else {
            zusdtxTracker->UpdateZerocoinMint(mint);
        }
    ...
    for (CDeterministicMint dMint : listDMints) {
        ...
        if (!zusdtxTracker->UnArchive(dMint.GetPubcoinHash(), true)) {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    CMintMeta meta = zusdtxTracker->Get(hashSerial);
    zusdtxTracker->SetPubcoinNotUsed(meta.hashPubcoin);
    return true;
}
...
bool CWallet::UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CZerocoinMint mint;
    if (zusdtxTracker->HasPubcoinHash(hashValue)) {
        CMintMeta meta = zusdtxTracker->GetMetaFromPubcoin(hashValue);
        meta.nHeight = nHeight;
        meta.txid = txid;
        return zusdtxTracker->UpdateState(meta);
    } else {
