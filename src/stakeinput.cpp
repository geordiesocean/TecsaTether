...
bool CUsdtxStake::InitFromTxIn(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend())
        return error("%s: unable to initialize CUSDTXStake from zerocoin spend", __func__);
...
bool CUsdtxStake::SetPrevout(CTransaction txPrev, unsigned int n)
{
    this->txFrom = txPrev;
    this->nPosition = n;
    return true;
...
bool CUsdtxStake::GetTxFrom(CTransaction& tx) const
{
    if (txFrom.IsNull())
        return false;
    tx = txFrom;
...
bool CUsdtxStake::GetTxOutFrom(CTxOut& out) const
{
...
bool CUsdtxStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(txFrom.GetHash(), nPosition);
    return true;
}

CAmount CUsdtxStake::GetValue() const
{
    return txFrom.vout[nPosition].nValue;
}

bool CUsdtxStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CUsdtxStake::GetUniqueness() const
{
    //The unique identifier for a USDTX stake is the outpoint
...
CBlockIndex* CUsdtxStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
bool CUsdtxStake::ContextCheck(int nHeight, uint32_t nTime)
{
