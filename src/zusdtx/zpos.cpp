...
bool CLegacyZUsdtxStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZUSDTXStake from non zc-spend", __func__);
    ...
    *this = CLegacyZUsdtxStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for zUSDTX stake origin", __func__);

    // All good
    return true;
}

CLegacyZUsdtxStake::CLegacyZUsdtxStake(const libzerocoin::CoinSpend& spend)
{
...
CBlockIndex* CLegacyZUsdtxStake::GetIndexFrom()
{
...
CAmount CLegacyZUsdtxStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZUsdtxStake::GetUniqueness() const
{
...
bool CLegacyZUsdtxStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zUSDTX stake block: height %d outside range", __func__, nHeight);
