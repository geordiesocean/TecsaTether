...
#include "zusdtx/zusdtxwallet.h"


CzUSDTXTracker::CzUSDTXTracker(CWallet* parent)
{
    this->wallet = parent;
...
CzUSDTXTracker::~CzUSDTXTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzUSDTXTracker::Init()
{
...
bool CzUSDTXTracker::Archive(CMintMeta& meta)
{
...
bool CzUSDTXTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzUSDTXTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzUSDTXTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzUSDTXTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzUSDTXTracker::GetSerialHashes()
{
...
CAmount CzUSDTXTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzUSDTXTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzUSDTXTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzUSDTXTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzUSDTXTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzUSDTXTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzUSDTXTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzUSDTXTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzUSDTXTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzUSDTXTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(wallet->strWalletFile);
...
void CzUSDTXTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzUSDTXWallet* zUSDTXWallet)
{
    bool iszUSDTXWalletInitialized = (nullptr != zUSDTXWallet);
    CMintMeta meta;
    ...
    if (!iszUSDTXWalletInitialized)
        zUSDTXWallet = new CzUSDTXWallet(wallet);
    meta.isSeedCorrect = zUSDTXWallet->CheckSeed(dMint);
    if (!iszUSDTXWalletInitialized)
        delete zUSDTXWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzUSDTXTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzUSDTXTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzUSDTXTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzUSDTXTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzUSDTXTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzUSDTXTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed, bool fExcludeV1)
{
    CWalletDB walletdb(wallet->strWalletFile);
    if (fUpdateStatus) {
        ...
        LogPrint(BCLog::LEGACYZC, "%s: added %d dzusdtx from DB\n", __func__, listDeterministicDB.size());
    }
