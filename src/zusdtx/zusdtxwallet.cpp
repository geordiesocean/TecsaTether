...
CzUSDTXWallet::CzUSDTXWallet(CWallet* parent)
{
    this->wallet = parent;
    ...
    //Check for old db version of storing zUSDTX seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZUSDTXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (wallet->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZUSDTXSeed_deprecated()) {
                    LogPrintf("%s: Updated zUSDTX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zUSDTX seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fRegtest && fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zUSDTX wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzUSDTXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZUSDTXCount(nCountLastUsed);
    else if (!walletdb.ReadZUSDTXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzUSDTXWallet::Lock()
{
    seedMaster.SetNull();
}

void CzUSDTXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzUSDTXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZUSDTX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzUSDTXWallet::LoadMintPoolFromDB()
{
...
void CzUSDTXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzUSDTXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzUSDTXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (std::pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (wallet->zusdtxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzUSDTXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZUSDTX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zUSDTXTracker which also adds to database
    wallet->zusdtxTracker->Add(dMint, true);
...
void CzUSDTXWallet::SeedToZUSDTX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
    libzerocoin::ZerocoinParams* params = Params().GetConsensus().Zerocoin_Params(false);
...
uint512 CzUSDTXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzUSDTXWallet::UpdateCount()
{
    nCountLastUsed++;
    CWalletDB walletdb(wallet->strWalletFile);
    walletdb.WriteZUSDTXCount(nCountLastUsed);
...
void CzUSDTXWallet::GenerateDeterministicZUSDTX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzUSDTXWallet::GenerateMint(const uint32_t& nCount, const libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzUSDTXWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzUSDTXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
