...
from test_framework.test_framework import TecsaTetherTestFramework
...
class ReorgStakeTest(TecsaTetherTestFramework):
    ...
    def check_money_supply(self, expected_usdtx, expected_zusdtx):
        g_info = [self.nodes[i].getinfo() for i in range(self.num_nodes)]
        # verify that nodes have the expected USDTX and zUSDTX supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_usdtx))
            for denom in node['zUSDTXsupply']:
                assert_equal(node['zUSDTXsupply'][denom], DecimalAmt(expected_zusdtx[denom]))
    ...
    def run_test(self):
        ...
        # Check USDTX and zUSDTX supply at the beginning
        # ------------------------------------------
        # zUSDTX supply: 2 coins for each denomination
        expected_zusdtx_supply = {
            "1": 2,
            "5": 10,
            "10": 20,
            "50": 100,
        ...
        # USDTX supply: block rewards minus burned fees for minting
        expected_money_supply = 250.0 * 330 - 16 * 0.01
        self.check_money_supply(expected_money_supply, expected_zusdtx_supply)
        ...
        # Verify that USDTX and zUSDTX supplies were properly updated after the spends and reorgs
        self.log.info("Check USDTX and zUSDTX supply...")
        ...
        expected_zusdtx_supply[str(spent_coin_0)] -= spent_coin_0
        expected_zusdtx_supply[str(spent_coin_1)] -= spent_coin_1
        expected_zusdtx_supply["total"] -= (spent_coin_0 + spent_coin_1)
        self.check_money_supply(expected_money_supply, expected_zusdtx_supply)
