...
class ZerocoinSpendTest(TecsaTetherTestFramework):
    ...
    def run_test(self):
        ...
        def check_balances(denom, zusdtx_bal, usdtx_bal):
            zusdtx_bal -= denom
            assert_equal(self.nodes[2].getzerocoinbalance()['Total'], zusdtx_bal)
            usdtx_bal += denom
            wi = self.nodes[2].getwalletinfo()
            assert_equal(wi['balance'] + wi['immature_balance'], usdtx_bal)
            return zusdtx_bal, usdtx_bal
        ...
        zusdtx_balance = self.nodes[2].getzerocoinbalance()['Total']
        assert_equal(balance, DecimalAmt(13833.92))
        assert_equal(zusdtx_balance, 6666)
        ...
        zusdtx_balance, balance = check_balances(denom_2, zusdtx_balance, balance)
        ...
        block_time = stake_4_blocks(block_time)
        self.check_tx_in_chain(0, txid)
        zusdtx_balance, balance = check_balances(denom_3, zusdtx_balance, balance)
        ...
        _, _ = check_balances(denom_1, zusdtx_balance, balance)
        self.log.info("--> VALID PUBLIC COIN SPEND (v3) PASSED")
